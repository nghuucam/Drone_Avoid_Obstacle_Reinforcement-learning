...
def main():
    ...
    model_path = r"C:\Users\buhbu\gym-pybullet-drones\gym_pybullet_drones\My_Coding\TestModel\D3QN\drone_model_d3qn_eposide400.pth"
    d3qn_agent.model.load_state_dict(torch.load(model_path))
    d3qn_agent.target_model.load_state_dict(d3qn_agent.model.state_dict())
        ...
        ############    Khi bị truncated hoặc terminated    ##################
            if done:
                ...
                if (truncated[0] or terminated or (action_count >= MAX_ACTIONS)):
                    ...
                    if WIN == "1": 
                        print("Màn chơi thành công tới đích !!!\n")
                        win_eposide +=1
